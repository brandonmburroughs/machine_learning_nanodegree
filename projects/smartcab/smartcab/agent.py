    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if random.random() <= self.epsilon:
                ...
                maxQ_action = [state_action for state_action, Q_value in self.Q[state].iteritems()
                               if Q_value == maxQ]

                action = maxQ_action[random.randint(0, len(maxQ_action) - 1)]
...
def run():
    ...
    sim = Simulator(env, update_delay=0.1, log_metrics=True, optimized=True, display=False)
    ...
    sim.run(n_test=25, tolerance=0.025)
